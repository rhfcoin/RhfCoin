...
// Copyright (c) 2011-2012 RHFCCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RHFCCoinTEST0\r");
            Send(hSocket, "WHO #RHFCCoinTEST0\r");
        } else {
            // randomly join #RHFCCoin00-#RHFCCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #RHFCCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #RHFCCoin%02d\r", channel_number).c_str());
        }

...
uint256 hashGenesisBlock("0xab05693db16220537f0a3695f97e12938bfd469f5dd51fc768143ee5418db737");
...
const string strMessageMagic = "RHFCCoin Signed Message:\n";
...
{
    // RHFCCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // RHFCCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "RHFC Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1481675999;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1295858;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x2be5b326f99632ba488efc985c10736d323d42cc9d6e78e8a5f9c09bcb760bd9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd7, 0xce, 0xa5, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RHFCCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RHFCCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RHFCCoinMiner(CWallet *pwallet)
{
    printf("RHFCCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("RHFCCoin-miner");
    ...
    try { loop {
        ...
        printf("Running RHFCCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RHFCCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RHFCCoinMiner, pwallet));
}
